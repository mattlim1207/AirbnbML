...
def assign():
    ...
    if len(db.engine.execute('''SELECT AVG(price) FROM listings WHERE neighbourhood = (%s)''', param).fetchall()) == 0:
...
def location(loc, amount):
    ...
    number = len(db.engine.execute("SELECT * FROM listings WHERE neighbourhood = (%s)", param).fetchall())
    price = db.engine.execute("SELECT AVG(price) FROM listings WHERE neighbourhood = (%s)", param).fetchone()[0]
    availability = int(db.engine.execute("SELECT AVG(availability_365) FROM listings WHERE neighbourhood = (%s)"
                                    , param).fetchone()[0])
